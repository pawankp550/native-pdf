...
export async function generateHtml(
  ...
  // Pre-generate barcode data URLs via canvas
...
function renderElement(
  ...
  switch (el.type) {
    ...
    case 'bullet-list':  return renderBulletList(el as BulletListElement, base);
    default:             return '';
  }
}
...
function renderRadio(el: RadioElement, base: string): string {
  ...
  const dotHtml = el.checked
    ? `<div style="width:${innerSize}px;height:${innerSize}px;border-radius:50%;background:${el.checkColor};"></div>`
    : '';
  ...
  const labelHtml = el.label
    ? `<span style="font-size:${el.labelFontSize}px;color:${el.labelColor};line-height:1.2;">${esc(el.label)}</span>`
    : '';
  const style = `${base}display:flex;flex-direction:${flexDir};align-items:center;gap:6px;`;
  return `<div class="el" style="${style}">${circleHtml}${labelHtml}</div>`;
}
...
function renderBarcode(el: BarcodeElement, base: string, barcodeDataUrls: Map<string, string>): string {
  ...
  if (!dataUrl) {
    return `<div class="el" style="${style};color:#9ca3af;font-size:11px;border:2px dashed #d1d5db;">Barcode</div>`;
  }
...
function renderBulletList(el: BulletListElement, base: string): string {
  const items = el.items.map((item, i) => {
    const bullet = el.bulletStyle === 'decimal' ? `${i + 1}.`
      : el.bulletStyle === 'disc'   ? '•'
      : el.bulletStyle === 'circle' ? '◦'
      : el.bulletStyle === 'square' ? '▪'
      : '';
    return `<div style="display:flex;align-items:flex-start;font-size:${el.fontSize}px;font-family:${el.fontFamily},Helvetica,sans-serif;color:${el.fontColor};font-weight:${el.fontWeight};line-height:${el.lineHeight};">
      <span style="color:${el.bulletColor};width:${el.indentSize}px;min-width:${el.indentSize}px;flex-shrink:0;text-align:center;">${esc(bullet)}</span>
      <span style="flex:1;word-break:break-word;">${esc(item)}</span>
    </div>`;
  }).join(`<div style="height:${el.gap}px;"></div>`);

  const style = `${base}background:${el.backgroundColor};padding:${el.padding}px;display:flex;flex-direction:column;overflow:hidden;box-sizing:border-box;`;
  return `<div class="el" style="${style}">${items}</div>`;
}
