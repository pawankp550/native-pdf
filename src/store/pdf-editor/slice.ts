  ...
  reducers: {
    ...
    resizeElement(state, action: PayloadAction<{ id: string; width: number; height: number; x?: number; y?: number }>) {
      ...
      if (el) {
        ...
        // For tables, scale column widths proportionally to match the new total width
        if (el.type === 'table' && el.columns.length > 0) {
          const currentColTotal = el.columns.reduce((s: number, c) => s + c.width, 0);
          const targetColTotal = action.payload.width - el.borderWidth;
          if (currentColTotal > 0 && Math.abs(targetColTotal - currentColTotal) > 0.5) {
            const scale = targetColTotal / currentColTotal;
            let distributed = 0;
            const lastIdx = el.columns.length - 1;
            el.columns.forEach((c, i) => {
              if (i === lastIdx) {
                c.width = Math.max(20, Math.round(targetColTotal - distributed));
              } else {
                c.width = Math.max(20, Math.round(c.width * scale));
                distributed += c.width;
              }
            });
          }
