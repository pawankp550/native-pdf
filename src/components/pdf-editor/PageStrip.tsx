...
export const PageStrip = React.memo(() => {
  ...
  return (
    <footer className="h-[72px] border-t bg-background flex items-center shrink-0">
      {/* Page list — scrollable, ~60% width */}
      <div className="flex items-center gap-2 px-3 overflow-x-auto shrink-0" style={{ width: '60%' }}>
      {pages.map((page, idx) => (
        <div
          key={page.id}
          className={`group relative flex-shrink-0 flex flex-col items-center justify-center w-14 h-[52px] rounded border-2 cursor-pointer transition-colors ${
            page.id === currentPageId ? 'border-primary bg-primary/5' : 'border-border hover:border-muted-foreground bg-card'
          }`}
