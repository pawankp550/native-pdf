...
import { Type, Minus, Square, Circle, CheckSquare, PenLine, Table2, ImageIcon, Hash, QrCode, CalendarDays, Heading, Link, Barcode, CircleDot, List } from 'lucide-react';
...
export const LeftPanel = React.memo(() => {
  ...
  const filteredIcons = q ? ALL_ICONS.filter(ic => ic.label.toLowerCase().includes(q)) : ALL_ICONS;

  return (
    <aside className="w-[220px] border-r bg-background flex flex-col shrink-0 overflow-y-auto">
      <div className="p-2 border-b sticky top-0 bg-background z-10">
        <Input placeholder="Search elements…" value={search} onChange={e => setSearch(e.target.value)} className="h-7 text-xs" />
      </div>
      <div className="p-2 flex flex-col gap-3">
        {filteredItems.length > 0 && (
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-foreground mb-1.5 px-1">Elements</p>
            <div className="grid grid-cols-1 gap-1.5">
              {filteredItems.map(item => <DraggableTile key={item.type} item={item} />)}
            </div>
          </div>
        )}
        {filteredIcons.length > 0 && (
          <div>
            ...
            <p className="text-[10px] font-bold uppercase tracking-wider text-foreground mb-1.5 px-1">Icons</p>
...
type GridCellComponentProps = { ariaAttributes: { 'aria-colindex': number; role: 'gridcell' }; columnIndex: number; rowIndex: number; style: React.CSSProperties } & IconCellProps;
...
const GridCell = ({ columnIndex, rowIndex, style, icons }: GridCellComponentProps) => {
  ...
  return <div style={style} className="p-0.5"><IconTile label={label} Icon={Icon} /></div>;
};
...
const IconGrid = React.memo(({ icons }: { icons: IconEntry[] }) => {
  ...
  return (
    <Grid<IconCellProps> cellComponent={GridCell} cellProps={{ icons }} columnCount={COLS} columnWidth={COL_W} rowCount={rowCount} rowHeight={ROW_H} defaultHeight={height} defaultWidth={GRID_W} style={{ height, width: GRID_W }} />
  );
});
...
const IconTile = React.memo(({ label, Icon }: { label: string; Icon: LucideIcon }) => {
  ...
  return (
    <div draggable onDragStart={onDragStart} className="flex flex-col items-center justify-center gap-0.5 p-1 rounded border border-border bg-card hover:bg-accent hover:border-primary cursor-grab active:cursor-grabbing transition-colors select-none h-full w-full" title={`Drag to add ${label}`}>
      <Icon size={12} strokeWidth={1.5} />
      <span className="text-[7px] text-center leading-tight line-clamp-2 w-full px-0.5">{label}</span>
    </div>
  );
});
