...
import {
    ...
    selectSelectedElementIds, selectZoom, selectShowGrid,
...
export const Canvas = React.memo(() => {
    ...
    // Track last drag-over position as fallback for drop (React synthetic drop event can have clientX/Y = 0)
    ...
    const handleDrop = useCallback((e: React.DragEvent<HTMLDivElement>) => {
        ...
        // Center element on cursor, clamped within canvas
        ...
        if (type === 'page-number') {
            // Distribute a copy to every page automatically
            dispatch(addPageNumberToAllPages(el as PageNumberElement));
            // Select the copy that landed on the current page (it has the original id)
    ...
    const handleElementMouseDown = useCallback((e: React.MouseEvent, el: CanvasElement) => {
        ...
        // If something is being edited, blur it first (triggers commit) and swallow the click
        if (editingId) {
    ...
    // Canvas mouse down (rubber band / deselect)
    const handleCanvasMouseDown = useCallback((e: React.MouseEvent) => {
        if (e.button !== 0) return;
        if (editingId) {
            // Blur the active element FIRST so onBlur fires while it's still mounted.
            // This lets onCommitText run and save the typed text before setEditingId(null) unmounts the div.
            // Do NOT call setEditingId(null) here directly — that would unmount the contentEditable
            // before the blur event fires, preventing onBlur/onCommitText from executing.
            (document.activeElement as HTMLElement | null)?.blur();
            return;
        }
        const pos = getCanvasPos(e);
    ...
    // Double-click to enter inline editing (text and table)
                        ...
                        return (
                            ...
                            >
                                <ElementRenderer
                                    ...
                                    onCommitTable={(data, columns) => {
                                        dispatch(updateElement({ id: el.id, changes: { data, columns } as Partial<typeof el> }));
                                        setEditingId(null);
                                    }}
                                />
