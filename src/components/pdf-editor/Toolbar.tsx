...
import { setZoom, setTemplateName, setShowGrid, undo, redo, markSaved, loadTemplateState, setBasePdf, clearBasePdf } from '@/store/pdf-editor/slice';
import { selectZoom, selectTemplateName, selectIsDirty, selectShowGrid, selectCanUndo, selectCanRedo, selectPages, selectElements, selectBasePdf } from '@/store/pdf-editor/selectors';
...
export const Toolbar = React.memo(({ darkMode, onToggleDark }: ToolbarProps) => {
  ...
  const handleExportPdf = async () => {
    setExporting(true);
    try {
      const bytes = await generatePdf(pages, elements, basePdf);
